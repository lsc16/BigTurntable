        ...
        <div class="lucky-wheel">
            ...
            <div class="wheel-main">
                ...
                <div class="wheel-bg" :style="rotateStyle">
                    ...
                    <div class="prize-dividingLineList">
                        <div v-for="(item,index) in angle"
                             :key="index"
                             :style="item"
                             class="prize-dividingLine"
                        ></div>
                    </div>
                </div>
            </div>
  ...
  export default {
    ...
    computed: {
      ...
      dividingLine() {
        return function(num) {
          let angle=[];
          var index = 0;//1、声明循环变量
          while (index<num){//2、判断循环条件;
            angle[index]={
              transform: `rotate(${index*(360/num)}deg)`,
            };
            index++;//4、更新循环变量；
          }
          this.angle=angle;
    ...
    methods: {
      initPrizeList() {
        // 这里可以发起请求，从服务端获取奖品列表
        // 这里使用模拟数据
        this.dividingLine(prizeList.length);
      ...
      rotateOver () {
        this.isRotating = false
        this.toastImg=prizeList[this.index].icon
